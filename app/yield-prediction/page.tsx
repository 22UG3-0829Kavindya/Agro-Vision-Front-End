...
export default function YieldPredictionPage() {
    ...
    const normalizeLiteral = (v: string) => {
        const m = v.trim().toLowerCase();
        const map: Record<string, string> = {
            low: "Low", medium: "Medium", high: "High",
            none: "None", organic: "Organic", inorganic: "Inorganic", mixed: "Mixed",
        };
        return map[m] ?? v;
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload = {
            plant_height_cm: Number(formData.plantHeight),
            stem_diameter_cm: Number(formData.stemDiameter),
            leaf_count: Number(formData.leafCount),
            plant_age_months: Number(formData.plantAge),
            soil_moisture: normalizeLiteral(formData.soilMoisture),
            avg_temp_c: Number(formData.temperature),
            fertilizer_application: normalizeLiteral(formData.fertilizer),
            cassava_variety: formData.variety,
            planting_density: Number(formData.plantingDensity),
        };

        setIsPredicting(true);
        try {
            const res = await fetch(`http://localhost:8000/yield/predict`, {
                method: "POST",
            ...
            const data: { predicted_yield_tph: number } = await res.json();
            setPrediction(data.predicted_yield_tph);
        } catch (err: any) {
                    ...
                    <Card className="mt-8 bg-green-50 border-green-200">
                        <CardContent className="p-6">
                            <p className="text-green-900 font-semibold">
                                {/* {t.predictedYield}: <span className="text-2xl">{prediction.toFixed(2)}</span> {t.tonnesPerHectare} */}
                                Predicted Yield: <span className="text-2xl">{prediction.toFixed(2)}</span> Tonnes per Hectare
                            </p>
                            <p className="text-green-700 text-sm mt-1">
                                {/* {t.disclaimer} */}
                                Disclaimer: This is a prediction and not a guarantee of actual yield.
                            </p>
                        </CardContent>
                    </Card>
                )}
